...
## 图文排版 API 效果优化
- [x] 需扸1：graphicLayoutService.ts Step1 layout_plan json_schema 新增 imagePrompt 字段，system prompt 加引导
- [x] 需扸2：Step2 imagePrompt 构建逻辑改为优先使用 LLM 输出的 plan.imagePrompt，仅回退时追加 imageGenStyleSuffix
- [x] 需扸4：文字区域描述从 "SOLID COLOR RECTANGLE" 改为更自然的背景纹理描述
- [x] 需扸3：openclawApi.ts 新增 POST /api/v1/media/generate 端点，复用 mediaRouter.generate 逻辑
- [x] 需扸3：openApiSpec.ts 补充 /api/v1/media/generate 的 OpenAPI spec
- [x] 验证：8 个 vitest 测试全部通过
