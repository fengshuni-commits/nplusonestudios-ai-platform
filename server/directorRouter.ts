...
async function executeTool(
  ...
  try {
    ...
    if (toolName === "search_building_codes") {
      ...
      try {
        const resp = await fetch(`${apiUrl}/search?query=${encodeURIComponent(query)}&top_k=${topK}`, {
          signal: AbortSignal.timeout(10000),
        });
        if (!resp.ok) return `规范搜索服务返回错误：${resp.status}`;
        const data = await resp.json() as { results?: Array<{ id: string; standard_name: string; clause_number: string; content: string; score: number }> };
        const results = data.results || [];
        if (results.length === 0) return `未找到与「${query}」相关的规范条文。`;
        const lines = results.map((r, i) =>
          `${i + 1}. **${r.standard_name}** 第${r.clause_number}条\n   ${r.content.trim()}`
        );
...
**规范引用原则**
- 当用户询问任何涉及建筑设计规范、强制性条文、设计标准（如净高、间距、无障碍、防火、采光、通风等）的问题时，必须先调用 search_building_codes 工具检索相关条文
- 在回答中明确引用条文来源，格式：「依据《规范名称》第X.X.X条：……」
- 如果搜索结果中有强制性条文（通常注明"强制性条文"），需特别标注⚠️
- 规范数据库包含65部规范，涵盖民用建筑、办公建筑、无障碍、防火、绿建等主要设计规范`;
}
