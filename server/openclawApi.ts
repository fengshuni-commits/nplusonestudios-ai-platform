...
// Generate social media content (text + cover image) for xiaohongshu, wechat, or instagram.
router.post("/media/generate", async (req: Request, res: Response) => {
  try {
    const user = (req as any).apiUser;
    if (!user) return res.status(401).json({ error: "Unauthorized", code: "UNAUTHORIZED" });

    const { platform, topic, projectName, style, referenceImageUrl, additionalNotes } = req.body;
    if (!platform || !topic) {
      return res.status(400).json({ error: "platform and topic are required", code: "VALIDATION_ERROR" });
    }
    const validPlatforms = ["xiaohongshu", "wechat", "instagram"];
    if (!validPlatforms.includes(platform)) {
      return res.status(400).json({ error: `platform must be one of: ${validPlatforms.join(", ")}`, code: "VALIDATION_ERROR" });
    }
...
Note: Instagram content should be visually driven, use professional English, include relevant architecture/design hashtags.`,
    ...
    try {
      const imagePrompt = textContent.coverImagePrompt || `${config.imagePrompt}, ${topic}`;
      const genOpts: Parameters<typeof generateImage>[0] = { prompt: imagePrompt };
      if (referenceImageUrl) {
        genOpts.originalImages = [{ url: referenceImageUrl, mimeType: "image/png" }];
      }
      const imageResult = await generateImage(genOpts);
      coverImageUrl = imageResult.url || null;
    } catch (imgErr) {
      console.error("[API] media/generate cover image failed:", imgErr);
    }

    // Step 3: Record in history
    const historyResult = await db.createGenerationHistory({
      userId: user.id,
      module: `media_${platform}`,
      title: `${config.name} - ${String(topic).substring(0, 40)}`,
      summary: String(topic).substring(0, 200),
      inputParams: { platform, topic, style },
      outputUrl: coverImageUrl,
      outputContent: JSON.stringify(textContent),
      status: "success",
    ...
    return res.json({
      data: {
        ...
        textContent,
        coverImageUrl,
        historyId: (historyResult as any).id,
      },
    });
  } catch (error) {
    console.error("[API] media/generate error:", error);
    res.status(500).json({ error: "Failed to generate media content", code: "INTERNAL_ERROR" });
  }
});

// ─── Webhook Trigger Helper ───────────────────────────────────────────────
async function triggerWebhook(event: string, payload: any) {try {
