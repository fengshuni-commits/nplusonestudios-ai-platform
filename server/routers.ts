...
const aiToolsRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const filtered = sanitized.filter((t: any) => {
        if (input.capability) {
          // Only use stored capabilities — do NOT merge with inferred ones.
          // inferCapabilities is only used at creation time to set initial values.
          // Merging at query time would override manual edits made by the admin.
          const stored: string[] = Array.isArray(t.capabilities) ? t.capabilities : [];
          return stored.includes(input.capability);
        }
  ...
  testTool: adminProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const reply = (result?.choices?.[0]?.message?.content ?? "(无返回内容)") as string;
      const model = (result?.model ?? String(input.toolId)) as string;
      return { success: true, latencyMs, model, reply };
    }),
});
...
const graphicLayoutRouter = router({
  ...
  // ─── 更新文字块位置（拖拽调整后保存） ────────────────────────────────────────────────────────────────────────────
  updateTextBlockPosition: protectedProcedure
    .input(z.object({
      jobId: z.number(),
      pageIndex: z.number(),
      blockId: z.string(),
      ...
      const updatedPages = pages.map((p: any) => {
        ...
        return {
          ...
          textBlocks: (p.textBlocks ?? []).map((b: any) =>
            b.id === input.blockId
              ? { ...b, x: input.x, y: input.y, width: input.width, height: input.height }
              : b
          ),
        };
      });
...
const expenseRouter = router({
  ...
  export: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      // Fetch all reports with items
      const reports = await Promise.all(
        input.reportIds.map(id => db.getExpenseReportById(id))
      );
      const validReports = reports.filter((r): r is NonNullable<typeof r> => r !== null);

      // ── Build Excel ────────────────────────────────────────────────
      const workbook = new ExcelJS.Workbook();
      workbook.creator = "N+1 STUDIOS";
      ...
      sheet.columns = [
        ...
        { header: "费用类别", key: "category", width: 16 },
      ...
      for (const report of validReports) {
        ...
        for (const item of report.items) {
          const invoices: { url: string; fileName: string }[] = item.invoicesJson
        ...
        // Subtotal row per report
        const subtotalRow = sheet.addRow({
          reportId: "",
          submitter: "",
          purpose: `小计（${report.purpose}）`,
          period: "",
          expenseDate: "",
          category: "",
          description: "",
      ...
      // Grand total row
      const grandTotal = validReports.reduce((s, r) => s + r.totalAmount, 0);
      const totalRow = sheet.addRow({
        reportId: "", submitter: "", purpose: "合计",
        period: "", expenseDate: "", category: "", description: "",
        projectName: "",
        amount: (grandTotal / 100).toFixed(2),
        invoiceCount: "", status: "", reviewNote: "",
      });
      ...
      // ── Build invoice ZIP ──────────────────────────────────────────
      const zip = new JSZip();
      const invoiceFolder = zip.folder("发票")!;
        ...
        for (const item of report.items) {
          ...
          for (const inv of invoices) {
            try {
              ...
              const ext = inv.fileName.includes(".") ? inv.fileName.split(".").pop() : "jpg";
              const safeName = `${String(invoiceSeq).padStart(3, "0")}_${report.id}_${item.description.slice(0, 10).replace(/[^\w\u4e00-\u9fa5]/g, "_")}.${ext}`;
              invoiceFolder.file(safeName, arrayBuf);
              invoiceSeq++;
            } catch {
      ...
      const [excelResult, zipResult] = await Promise.all([
        storagePut(
          `expense-exports/${nanoid()}-费用清单-${timestamp}.xlsx`,
          Buffer.from(excelBuffer),
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        ),
        storagePut(
          `expense-exports/${nanoid()}-发票-${timestamp}.zip`,
          zipBuffer,
          "application/zip"
        ),
      ]);
